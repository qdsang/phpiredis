...
PHP_FUNCTION(phpiredis_pconnect)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|l|l", &ip, &ip_size, &port, &msec) == FAILURE) {
        return;
    }
    ...
    //c = redisConnectWithTimeOut(ip, (int)port, tv); // FIXME: unsafe cast
...
PHP_FUNCTION(phpiredis_command)
{
    ...
    while (reply == NULL || reply->type == REDIS_REPLY_ERROR)
    {
        ...
        if (reply == NULL) {
            redisFree(connection->c);
            //c = redisConnectWithTimeout(connection->ip, connection->port, tv);
            c = redisConnect(connection->ip, connection->port);
...
PHP_MINFO_FUNCTION(phpiredis) /* {{{ */
{
	php_info_print_table_start();
	php_info_print_table_header(2, "phpiredis", "enabled");

	php_info_print_table_row(2, "Version", "1.0");
	php_info_print_table_row(2, "build", "funshion fl");
